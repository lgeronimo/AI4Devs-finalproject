...
@Component({
  selector: 'app-voice-command-iframe',
  standalone: true,
  imports: [CommonModule, OverlayModule],
  templateUrl: './voice-command-iframe.component.html',
  styleUrls: ['./voice-command-iframe.component.scss']
})
export class VoiceCommandIframeComponent implements OnInit, OnDestroy {
  ...
  private readonly commandActions = {
    bottom: () => this.webContentService.requestBottomPage(),
    top: () => this.webContentService.requestTopPage(),
    up: () => {
      this.lastAction = 'up';
      this.webContentService.requestUpPage();
    },
    down: () => {
      this.lastAction = 'down';
      this.webContentService.requestDownPage();
    },
    more: () => {
  ...
  closeComponent(): void {
    this.webContentService.clearViewerState();
